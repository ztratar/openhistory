  ...
  return (
    <main className="policy-page">
      <article>
        ...
        <p>Raw activity, summaries, settings, encrypted API keys, and local-agent credentials stay in a permission-restricted directory on your Mac. During first-run setup, email and messaging activity are selected for inclusion by default; you can clear either choice before finishing setup and control both independently later. The OpenHistory app does not operate analytics or telemetry. You can pause capture, disable capture categories, exclude apps, inspect the folder, export content-free diagnostics, or permanently delete all local OpenHistory data.</p>

        <h2>Website analytics</h2>
        <p>This marketing website uses Google Analytics to measure page visits and interactions such as navigation, downloads, and outbound-link clicks. Website analytics do not include activity collected by the OpenHistory Mac app. Click events use predefined labels and destination paths; URL query parameters and arbitrary page text are not sent as click-event fields.</p>
